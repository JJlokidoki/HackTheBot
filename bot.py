...
#        self.refresh_ippsec.start()
    ...
    @tasks.loop(seconds=30.0) #Toutes les 10 secondes, check les notifications
...
async def get_box(ctx, *, content=""):
    """Информацию по машине (Example: !get_box Openadmin)"""
    # Args parser
    args = content.split()
    count = 0
    matrix = False
    ...
    if target:
        ...
        if box:
            ...
            await ctx.send("Машина не найдена.")
...
async def last_box(ctx, param=""):
    """Показать самую новую машину (Можно дописать флаг -m)"""
    if param:
...
async def list_boxes(ctx, *, content=""):
    """Список активных машин по сложности (Example: !list_boxes insane)"""
...
async def list_challs(ctx, *, content=""):
    """Список категорий тасков (Example: !list_challs Reversing)"""
...
async def account(ctx, arg, action=""):
    """Регистрация в боте (Далее следовать инструкции в ЛС)"""
...
async def progress(ctx, *, content=""):
    """Прогресс по машине или таску (Example: !progress Openadmin или Templed)"""

    async def make_embed(ctx, target, box=False, chall=False):
        guilds = bot.guilds
        print(' '.join([str(i.name) for i in guilds]))
        bot_guild = False
        for guild in guilds:
            if guild.name == cfg.discord['guild_name']:
                bot_guild = guild
    ...
    if target:
        if box:
            ...
            if box_status:
                ...
                print('\033[96m' + str(ctx) + "\n" + str(type(ctx)) + "\n" + str(ctx.message) + str(progress_stats) + '\033[0m')
...
async def man(ctx, command=""):
    "Справка по командам (Example: !man account"
    ...
    if command == "man" or not command:
        ...
        ***Информация о пользователе HTB (Example: !get_user GreyParzival)***
